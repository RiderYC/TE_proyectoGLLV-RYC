...
public class VentaDAOimpl extends ConexionDB implements VentaDAO {
    ...
    public void insert(Venta venta) throws Exception {
        try {
            this.conectar();
            String sql = "INSERT INTO ventas (nombre, producto_id, cliente_id, tipo, fecha) values (?,?,?,?,?)";
            PreparedStatement ps = this.conn.prepareStatement(sql);

            ps.setString(1, venta.getNombre());
            ps.setInt(2, venta.getProducto_id());
            ps.setInt(3, venta.getCliente_id());
    ...
    public void update(Venta venta) throws Exception {
        try {
            this.conectar();
            String sql = "UPDATE ventas SET nombre=?, producto_id = ?, cliente_id = ?, tipo =?, fecha = ? WHERE id = ?";
            PreparedStatement ps = this.conn.prepareStatement(sql);
